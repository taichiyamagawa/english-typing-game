...
- 記事を書き終えたら、本文中の**すべての単語**（専門語・内容語・一般的な動詞・形容詞・副詞・慣用句）を洗い出し、未登録のものを vocabulary.ts に追記する
  - 「専門語・難しい単語だけ追加」では不十分。基本的な英単語も日本語訳があることで学習効果が上がるため、**一般語も積極的に追加する**
  - 例：`survive`, `gradually`, `entire`, `apparently` なども未登録なら追加対象
- すでに登録済みの単語は重複追加しない（追加前に `grep` で既存キーを確認する）
