...
### 語彙カバー率のルール
- 記事を新規作成したら、**必ず** `data/vocabulary.ts` に未登録語を追加し、**記事全体の単語・熟語カバー率90%以上**を達成してから完了とする
- 専門語だけでなく、一般的な動詞・形容詞・副詞・名詞もすべて追加対象（`find`, `real`, `human` なども未登録なら追加する）
- 追加前に vocabulary.ts の既存キーと照合し、重複しない語だけを追加する（重複はTypeScriptエラーになる）
